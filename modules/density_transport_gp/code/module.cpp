...
void register_density_transport_gp(ModuleRegistry& r) {
    r.add("density_transport_gp", [](StageContext& x,const Json& j) {
        ...
        return nonlocal::run(x,j,[coarse](const ea::Database& db,ea::ExactOverlapDensity& d,
            ...
            for(int id:db.movable_ids){const auto& n=db.nodes[id];int X=std::clamp(int((n.x-db.xl)/(db.xh-db.xl)*cx),0,cx-1),Y=std::clamp(int((n.y-db.yl)/(db.yh-db.yl)*cy),0,cy-1);int s=Y*cx+X;if(sink[s]<0)continue;int t=sink[s];ax[id]=double(t%cx-X);ay[id]=double(t/cx-Y);}
        });
    });
}
}
