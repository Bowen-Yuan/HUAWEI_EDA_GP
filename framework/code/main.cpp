...
void csv(std::ofstream& f,int i,const std::string& mod,const M& before,const M& after,double secs,int its){f<<i<<','<<mod<<','<<std::setprecision(14)<<before.hpwl<<','<<after.hpwl<<','<<before.overflow<<','<<after.overflow<<','<<its<<','<<secs<<"\n";}
...
void write_stage(const fs::path& dir, ea::Database& db, const M& m, const std::string& mod, int idx, double sec) {
  ...
  Json manifest={{"module",mod},{"stage_index",idx},{"runtime_seconds",sec},{"metrics",{{"hpwl",m.hpwl},{"density_energy",m.energy},{"overflow",m.overflow},{"max_density",m.maxd}}}};
...
void run_case(const Opt& o, const std::string& cas) {
  ...
  std::ofstream summary(root/"case_summary.csv"); summary<<"stage,module,hpwl_before,hpwl_after,overflow_before,overflow_after,iterations,runtime_seconds\n";
